...
export default function SettingsDialog({ visible, onClose }: SettingsProps) {
  const { register } = useForm<SettingsFormProps>({
    defaultValues: {
      ...
      <DialogContent dividers={scroll === 'paper'}>
        <Box sx={{ flexGrow: 1 }}>
          <Grid container spacing={2}>
            ...
            />
            <CustomNumberInput placeholder="port number" className="w-max" defaultValue={8000} disabled />
          </Grid>
        </Box>
      </DialogContent>
      <DialogActions>
        <Button onClick={handleClose}>Cancel</Button>
        <Button disabled sx={{ ":disabled": { pointerEvents: 'none', opacity: 50 } }}>Apply</Button>
        <Button disabled sx={{ ":disabled": { pointerEvents: 'none', opacity: 50 } }}>Confirm</Button>
      </DialogActions>
    </Dialog>
  );
}
